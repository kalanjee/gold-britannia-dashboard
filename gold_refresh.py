...
def inject_data_into_dashboard(data):
    """Replace the DASHBOARD_DATA in the HTML file with live data."""
    with open(DASHBOARD_OUTPUT) as f:
        html = f.read()

    # Find and replace the DASHBOARD_DATA block
    marker_start = "const DASHBOARD_DATA = {"
    ...
    if start_idx == -1 or end_idx == -1:
        log.error("Could not find DASHBOARD_DATA markers in HTML — writing fresh file")
        # Alternative: inject as a separate script block at the end
        inject_point = html.rfind("</script>")
        if inject_point != -1:
            data_js = f"\n// === LIVE DATA (injected by gold_refresh.py) ===\nconst LIVE_DATA = {json.dumps(data, indent=2)};\n"
            html = html[:inject_point] + data_js + html[inject_point:]
    else:
        # Generate JavaScript-compatible data
        # We need to rebuild the history section without the helper functions
        data_js = json.dumps(data, indent=4)
        replacement = f"const DASHBOARD_DATA = {data_js};"
        html = html[:start_idx] + replacement + html[end_idx + len(marker_end) - 1:]

        # Remove the helper functions that generated sample data
        helpers_start = html.find("// Data generation helpers")
        helpers_end = html.find("// ========== CHART COLORS ==========")
        if helpers_start != -1 and helpers_end != -1:
            html = html[:helpers_start] + "\n" + html[helpers_end:]

    with open(DASHBOARD_OUTPUT, 'w') as f:
        f.write(html)

    log.info(f"Dashboard updated: {DASHBOARD_OUTPUT}")
